...
fn main() {
    let app = Application::builder()
        .application_id("org.mavensgroup.cview")
        .build();
...
fn build_ui(app: &Application) {
    ...
    // Tab 1: Structure Info (was "Interactions")
    let info_view = TextView::builder()
        .editable(false)
        ...
        .bottom_margin(10)
        .build();
    let scroll_info = ScrolledWindow::builder().child(&info_view).build();
    console_notebook.append_page(&scroll_info, Some(&Label::new(Some("Structure Info"))));
    ...
    // Initialize the Rust `log` crate logger → System Log tab
    if let Err(e) = utils::logger::init(&log_view) {
        eprintln!("Failed to initialize logger: {}", e);
    }

    // Initialize the centralized console system
    console::init(&info_view, &log_view);

    console::log_info("CView started.");

    let scroll_logs = ScrolledWindow::builder().child(&log_view).build();
    console_notebook.append_page(&scroll_logs, Some(&Label::new(Some("System Log"))));
    ...
    sidebar_widget.set_width_request(300);
    sidebar_widget.set_hexpand(false);
