...
use gtk4::prelude::*;
use gtk4::{
  Align, Button, FileChooserAction, FileChooserNative, FileFilter, Frame, Grid, Label, Orientation,
...
use plotters::style::TRANSPARENT; // Needed for invisible text anchors
use plotters_cairo::CairoBackend;

// Calamine 0.25+ imports
  ...
  DB::ErrorType: 'static,
{
  ...
  // We filter out very small peaks to avoid clutter
  let visible_peaks: Vec<&XRDPattern> = peaks
    .iter()
    .filter(|p| {
      p.two_theta >= settings.min_2theta && p.two_theta <= settings.max_2theta && p.intensity > 1.5
    })
    .collect();

  chart.draw_series(visible_peaks.iter().enumerate().map(|(i, p)| {
    let (h, k, l) = p.hkl[0]; // Use the first equivalent index
    let label = format!("({} {} {})", h, k, l);

    // Smart Height: Stagger labels to avoid overlap if peaks are close
    // Even indices: 15px up, Odd indices: 28px up
...
fn parse_excel_local(path: &std::path::Path) -> Result<ExperimentalData, String> {
  ...
  // Heuristic: Look for two columns of numbers. Skip headers automatically.
