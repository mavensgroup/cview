...
use gtk4::{ApplicationWindow, EventControllerScroll, EventControllerScrollFlags, GestureDrag, GestureClick, PropagationPhase};
use gtk4::glib;
use std::rc::Rc;
...
pub fn setup_interactions(
    ...
    scroll.connect_scroll(move |_, _, dy| {
        let mut st = s.borrow_mut();
        if dy > 0.0 { st.zoom *= 0.9; } else { st.zoom *= 1.1; }
        da.queue_draw();
