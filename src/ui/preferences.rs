...
fn build_export_tab(state: Rc<RefCell<AppState>>) -> gtk::Box {
    ...
    // Set initial selection
    let is_unit_cell = matches!(state.borrow().rotation_mode, RotationCenter::UnitCell);
    ...
    dropdown_rot.connect_selected_notify(move |d| {
        ...
        st.rotation_mode = match idx {
            1 => RotationCenter::UnitCell,
            _ => RotationCenter::Centroid,
        };
        st.save_config();
        println!("Preference saved: Rotation Mode = {:?}", st.rotation_mode);
    });
