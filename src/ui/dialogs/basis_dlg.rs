...
/// Resolves the selected unique_ids from interaction state back to original_indices
/// in structure.atoms, using a scene recalculation. Returns deduplicated, sorted indices.
fn resolve_selected_original_indices(state: &AppState) -> Vec<usize> {
  let tab = state.active_tab();
  let selected_uids: HashSet<usize> = tab.interaction.selected_indices.clone();

  if selected_uids.is_empty() {
    return vec![];
  }

  // Screen size doesn't affect original_index assignment, only screen_pos coords
  let (atoms, _, _) = scene::calculate_scene(tab, &state.config, 800.0, 600.0, false, None, None);

  let mut original_indices: HashSet<usize> = HashSet::new();
  for atom in &atoms {
    if selected_uids.contains(&atom.unique_id) {
      original_indices.insert(atom.original_index);
    }
  }

  let mut result: Vec<usize> = original_indices.into_iter().collect();
