...
pub fn calculate_scene(
    ...
    for x in 0..=1 {
        for y in 0..=1 {
            for z in 0..=1 {
                let fx = x as f64; let fy = y as f64; let fz = z as f64;
    ...
    for (i, atom) in structure.atoms.iter().enumerate() {
        let positions = if let Some(inv) = inv_lattice {
            ...
            let dx_list = if fx.abs() < 0.05 { vec![0.0, 1.0] } else { vec![fx] };
            let dy_list = if fy.abs() < 0.05 { vec![0.0, 1.0] } else { vec![fy] };
            let dz_list = if fz.abs() < 0.05 { vec![0.0, 1.0] } else { vec![fz] };
            ...
            for &dx in &dx_list {
                for &dy in &dy_list {
                    for &dz in &dz_list {
                        ...
                        let is_ghost = (dx - fx).abs() > 0.01 || (dy - fy).abs() > 0.01 || (dz - fz).abs() > 0.01;
        ...
        for (pos, ghost) in positions {
            ...
            if r_pos[0] < min_x { min_x = r_pos[0]; }
            if r_pos[0] > max_x { max_x = r_pos[0]; }
            if r_pos[1] < min_y { min_y = r_pos[1]; }
            if r_pos[1] > max_y { max_y = r_pos[1]; }
    ...
    let win_cx = if is_export { export_w / 2.0 } else { win_w / 2.0 };
...
fn get_rotation_center(state: &AppState) -> [f64; 3] {
    if let Some(s) = &state.structure {
        ...
        if n > 0.0 { return [sum[0]/n, sum[1]/n, sum[2]/n]; }
    }
    [0.0; 3]
}
